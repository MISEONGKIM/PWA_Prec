...
- 브라우저가 백그라운드에서 실행하는 스크립트로 웹페이지와는 별개로 동작하며 브라우저와 웹 서버 간의 미들웨어 역활을 수행합니다.

* 데이터캐싱을 통하여 오프라인에서도 정상적으로 앱이 동작하도록 도와줄 수 있고, 데이터 패칭을 통하여 성능최적화를 이룰 수 있다.

* 서비스워커는 https 환경 혹은 localhost 환경에서만 동작하기 때문에 local 환경에서 특정 도메인을 반드시 사용해야 하는경우 이에 대해서도 반드시 https 설정을 해야 함.
* 서비스워커(serviceWorker) 에서는 기존의 자바스크립트와 다른 쓰레드에서 사용되기때문에 self 를 통해 this 를 접근할 수 있음.

- PWA Builder에서는 미리 만들어진 서비스 워커 패키지들을 제공함.

  - Offline Pages: "offline.html"이라는 웹 서버에서 파일을 가져오고(파일이 실제로 있는지 확인) 네트워크 연결을 할 수 없을 때마다 파일을 제공.

  - Offline Page Copy of Pages: 앱의 오프라인 기능을 확장하는 솔루션. 방문자가 볼 때 각 페이지의 사본이 캐시에 저장됨. 이를 통해 방문자는 오프라인 상태에서 이전에 본 페이지를 로드할 수 있음.

  - Offline Copy with Backup Offline Page: 방문자가 각 페이지를 방문할 때마다 해당 페이지의 사본이 캐시에 저장됨. 이를 통해 방문자는 오프라인 상태에서 이전에 본 페이지를 로드할 수 있음. 앱 오프라인이고 페이지 캐시안되어 있으면 오프라인 페이지 추가함
